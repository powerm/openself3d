...
def create_non_correspondences(uv_a_matches, uv_b_matches, img_b_shape, num_non_matches_per_match=100, img_b_mask=None, img_b_depth=None):
    ...
    if img_b_mask is not None:
        if img_b_depth is not None:
            mask_b_indices = np.where(img_b_mask>0, 1,0)
            depth_b_indices = np.where(img_b_depth>0, 1,0)
            sythn_b_indices = np.where((mask_b_indices & depth_b_indices)>0)
        else:
            sythn_b_indices = np.where(img_b_mask>0)
        if sythn_b_indices[0].size ==0:
            print("warning, empy mask b")
            uv_b_non_matches = get_random_uv_b_non_matches()
        else:
            num_samples = num_matches*num_non_matches_per_match
            rand_numbers_b = np.random.rand(num_samples)*len(sythn_b_indices[0])
            rand_indices_b = np.floor(rand_numbers_b).astype(np.int64)
            uv_b_non_matches = (sythn_b_indices[0][rand_indices_b], sythn_b_indices[1][rand_indices_b])
    else:
        uv_b_non_matches = get_random_uv_b_non_matches()
    ...
    uv_b_non_matches =  (uv_b_non_matches[0].reshape(num_matches, num_non_matches_per_match), 
    ...
    copied_uv_b_matches_0 = np.tile(uv_b_matches[0],(num_non_matches_per_match, 1)).T
    copied_uv_b_matches_1 = np.tile(uv_b_matches[1], (num_non_matches_per_match,1)).T
    ...
    diffs_0 = np.abs(diffs_0)
    diffs_1 = np.abs(diffs_1)
    num_pixels_too_close = 1.0
    threshold = np.ones_like(diffs_0)*num_pixels_too_close
    
    # determine  which pixels are too close to being matches
    need_to_be_perturbed = ( np.where(diffs_0 < threshold,  1,  0) | np.where(diffs_1<threshold, 1, 0) )
    
    minimal_perturb  = num_pixels_too_close/2
    
    minimal_perturb_vector = np.floor(np.random.rand(need_to_be_perturbed.shape[0], need_to_be_perturbed.shape[1])*2)*(minimal_perturb*2) -minimal_perturb
    std_dev = 10
    random_vector = np.random.randn(need_to_be_perturbed.shape[0], need_to_be_perturbed.shape[1])*std_dev + minimal_perturb_vector
    perturb_vector = need_to_be_perturbed * random_vector
    
    uv_b_non_matches_0 = uv_b_non_matches[0] + perturb_vector
    uv_b_non_matches_1 = uv_b_non_matches[1] + perturb_vector
    
    # now just need to wrap around and that went out of bounds
    
    # handle wraping in height
    lower_bound = 0.0
    upper_bound = image_height*1.0 -1
    lower_bound_vec = np.ones_like(uv_b_non_matches_0)*lower_bound
    upper_bound_vec = np.ones_like(uv_b_non_matches_0)*upper_bound
    uv_b_non_matches_0 = where(uv_b_non_matches_0> upper_bound_vec, 
                                 uv_b_non_matches_0- upper_bound_vec, uv_b_non_matches_0)
    uv_b_non_matches_0 = where(uv_b_non_matches_0< lower_bound_vec, 
                                 uv_b_non_matches_0 + upper_bound_vec, uv_b_non_matches_0)
    
    # handle wrapping in height
    lower_bound = 0.0
    upper_bound = image_width*1.0 - 1
    lower_bound_vec = np.ones_like(uv_b_non_matches_1) * lower_bound
    upper_bound_vec = np.ones_like(uv_b_non_matches_1) * upper_bound

    uv_b_non_matches_1 = where(uv_b_non_matches_1 > upper_bound_vec, 
...
def mask_image_from_uv(matches_a, image_shape):
    image = np.zeros(image_shape)
    matches = (matches_a[1], matches_a[0])
    image[matches] = 1
    return  image
    


def create_blind_non_correspondences(uv_a,  image_a_mask, image_b_mask):
    """need modified,  this need to modified, because There is some

    Args:
        uv_a (_type_): _description_
        image_a_mask (_type_): _description_
        image_b_mask (_type_): _description_

    Returns:
        _type_: _description_
    """

    matches_a_mask = mask_image_from_uv(uv_a,  image_a_mask.shape)
    image_a_mask_float = image_a_mask.copy().astype(np.int64)
    blind_non_matches_a = np.where((image_a_mask_float- matches_a_mask)!=0)
    
    no_blind_matches_found = False
    if len(blind_non_matches_a[0]) == 0:
        no_blind_matches_found = True 
    else:
        num_blind_samples = len(blind_non_matches_a[0])
        blind_uv_b = random_sample_from_masked_image_numpy(image_b_mask, num_blind_samples)
        if len(blind_uv_b[0])==0:
            no_blind_matches_found = True
        elif blind_uv_b[0] is None:
            no_blind_matches_found = True
        else:
